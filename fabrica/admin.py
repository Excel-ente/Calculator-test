...
class InsumoAdmin(ImportExportModelAdmin):
    list_display = ('Nombre_de_producto','PROVEEDOR','Costo','Costo_unitario')#'Stock_actual'
    ordering = ('PRODUCTO',)
    list_filter = ('PROVEEDOR','PRODUCTO',)
    ...
    def Costo_unitario(self, obj):
        formateo = "$ {:,.2f}".format(obj.COSTO_UNITARIO)
        return formateo
...
class RecetaAdmin(admin.ModelAdmin):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "CATEGORIA":
            ...
            kwargs["empty_label"] = "Seleccione una categoría"  # Opcional: muestra un mensaje predeterminado
        return super().formfield_for_foreignkey(db_field, request, **kwargs)
